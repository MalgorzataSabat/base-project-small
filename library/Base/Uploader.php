...
class Base_Uploader
{
    ...
    public function finishUploadHash($finalName)
    {
        $hashFileName = Base::getHash().'.'.Base::getFileExt($finalName);

        $result = $this->finishUpload($hashFileName);
    ...
    public function finishUpload($finalName) {
        if(rename($this->getTempDirectory() . $this->getTempName(), $this->getMainDirectory() . $finalName)) {
            return array('errorStatus' => 0, 'name' => $finalName, 'filename' => $finalName, 'finish' => 1);
        }
        else {
            return array(
                'errorStatus' => 1,
