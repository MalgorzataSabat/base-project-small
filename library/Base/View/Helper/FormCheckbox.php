...
class Base_View_Helper_FormCheckbox extends Zend_View_Helper_FormElement
{
    /**
    ...
    public function formCheckbox($name, $value = null, $attribs = null, array $checkedOptions = null)
    {
        ...
        $attribs['class'].= ' checkbox style-2';
        $labelSpan = '';

        if(isset($attribs['text'])){
            $labelSpan .= ' <span>'.(isset($attribs['text']) ? $attribs['text'] : '').'</span>';
            unset($attribs['text']);
        } else {
            $labelSpan .= '<span></span>';
        }
        ...
        $xhtml = '<div class="'.$checkbox_class.'"><label for="'.$this->view->escape($id).'">'.$xhtml.'</label></div>';
